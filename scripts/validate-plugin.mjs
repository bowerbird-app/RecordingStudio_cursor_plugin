  ...
  if (await pathExists(skillsDir)) {
    ...
    for (const file of files) {
      if (path.basename(file) === "SKILL.md") {
        ...
        if (parsed?.name && parsed.name !== parentDir) {
          addError(
            `${pluginName}: skill folder "${parentDir}" must match frontmatter name "${parsed.name}" (${path.relative(repoRoot, file)})`
          );
        }
      }
    }
...
async function main() {
  ...
  if (!(await pathExists(readmePath))) {
    addError(`${pluginName}: README.md is required for marketplace submission.`);
  }
