...
2. For a top-level object, confirm `RecordingStudio.root_allowed?` and call `RecordingStudio.root_recording_for`.
3. For a child, resolve `parent_recording` and verify `RecordingStudio.parent_allowed?(child_type:, parent_recording:)`.
4. Prefer `record`, `revise`, and `log_event!` on `RecordingStudio::Recording`.

Recordables are immutable snapshots. History is append-only through events.
...
- Code matches Rails and repo patterns.
- Relevant tests pass, or blockers are documented with logs.
